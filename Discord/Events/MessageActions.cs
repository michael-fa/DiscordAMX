...
namespace discordamx.Discord.Events
{
    public static class MessageActions 
    {
        ...
        {
            if (arg.Author == Discord.Bot.Client.CurrentUser) return Task.CompletedTask;
        ...
        {
            if (Utils.UserBans.IsBanned(c.CurrentUser.Id.ToString())) return Task.CompletedTask; //Maybe incorrect user but I am uncertain for now.
            ...
            if (arg.Message.Channel.IsPrivate)
            {
                ...
                if (!Program.m_DmUsers.Contains((DiscordDmChannel)Discord.Bot.Client.GetChannelAsync(arg.Message.ChannelId).Result))
